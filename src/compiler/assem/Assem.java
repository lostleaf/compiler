...
public class Assem implements Config {
	...
	private boolean isSpill() {
		return true;
		/*
		 * for (Object param : params) { if (param instanceof Temp && ((Temp)
		 * param).getLiveInterval().spilled) { return true; } } return false;
		 */
	}
	...
	private String doSpill(RegAlloc allocator) {
		...
		for (Temp t : use()) {
			if (t.getLiveInterval().register == spillReg) {
				...
				// before.append("\t\t\tlw $" + regNames[r] + ", " + wordSize *
				// t.home.depth + "($gp)\t# load display for spilling\n");
				// before.append("\t\t\tlw $" + regNames[r] + ", " + wordSize *
				// t.index + "($" + regNames[r] + ")\t# load for spilling\n");
				before.append("\t\t\tlw $" + regNames[r] + ", "
						+ (wordSize * t.index + t.level.size)
		...
		for (Temp t : def()) {
			if (t.getLiveInterval().spilled
				...
				// after.append("\n\t\t\tlw $" + regNames[d] + ", " + wordSize *
				// t.home.depth + "($gp)\t# load display for spilling");
				// after.append("\n\t\t\tsw $" + regNames[r] + ", " + wordSize *
				// t.index + "($" + regNames[d] +
				// ")\t# write back for spilling");
				after.append("\n\t\t\tsw $" + regNames[r] + ", "
						+ (wordSize * t.index + t.level.size)
						+ "($sp)\t# write back for spilling");
			}
		}
